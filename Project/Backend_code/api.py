...
def getData():
    try:
        ...
        data=list(data)
        #i=jsonify(dumps(data))
        return {"data":data,"message":"Successfully Fetched"},200
...
@app.route('/api/putlocation' , methods=['POST'])
def putLocation():
    try:
        location = request.get_json().get('location')
        i1 = coord(location)
        data = {'name': location, 'latitude': i1[0], 'longitude': i1[1]}
        collection4.insert_one(data)
        return jsonify({"message": "User data inserted successfully", "Coordinates": i1}), 201
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
with app.test_client() as client:
    # You can now use client to make requests
